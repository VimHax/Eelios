	...
	public print(contents: string): void {
		switch (this.kind) {
			case SyntaxErrorKind.InvalidCharacter: {
				...
				console.log(`Found an invalid character, "${char}", at ${span.print(contents)}`);
				break;
			}
			case SyntaxErrorKind.UnterminatedStringLiteral: {
				const [str, span] = this.info as [string, Span];
				console.log(`Found an unterminated string literal, "${str}", at ${span.print(contents)}`);
				break;
			}
			case SyntaxErrorKind.ExpectedButFound: {
				const [kind, token] = this.info as [TokenKind[], Token];
				console.log(`Expected a ${kind} token, but found a ${token.getKind()}, at ${token.getSpan().print(contents)}`);
				break;
			}
		}
	}
